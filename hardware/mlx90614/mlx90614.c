...
{
  u8 Bit_counter,Ack_bit,bit_out;
...
void SMBus_Init()
{
   ...
   __HAL_RCC_GPIOB_CLK_ENABLE();//非接触温度传感器SDAL 连接PB.15,SCL连接PB.14---打开GPIOD时钟
...
repeat:
     ...
     if(!ErrorCounter)break;  //ErrorCounter=0?//Yes,go out from do-while{}
...
{   
	float temp;
	temp = SMBus_ReadMemory(SA, RAM_ACCESS|RAM_TOBJ1)*0.02f-273.15f;
	return temp;
}
